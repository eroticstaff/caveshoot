...
void ProcessEvent(const sf::Event &event) {
  if (s_windowHasFocus) {
    ...
    case sf::Event::JoystickDisconnected:
      if (s_joystickId ==
          event.joystickConnect.joystickId) { // used gamepad was disconnected
        s_joystickId = getConnectedJoystickId();
      }
      break;
    default:
...
void Image(const sf::Sprite &sprite, const sf::Vector2f &size,
  ...
  Image(*texturePtr, size, static_cast<sf::FloatRect>(sprite.getTextureRect()),
